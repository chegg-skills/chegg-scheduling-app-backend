...
export function useLanguages() {
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      configApi.getLanguages(signal).then((r) => r.data?.data?.languages ?? []),
    staleTime: Infinity,
  })
}
...
export function useCountries() {
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      configApi.getCountries(signal).then((r) => r.data?.data?.countries ?? []),
    staleTime: Infinity,
  })
}
...
export function useTimezones() {
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      configApi.getTimezones(signal).then((r) => r.data?.data?.timezones ?? []),
    staleTime: Infinity,
  })
}
