...
import { useWeeklyAvailability, useAvailabilityExceptions } from '@/hooks/queries/useAvailability'
...
export function UserCalendarTab({ user }: UserCalendarTabProps) {
  ...
  const {
    ...
    error: errorWeekly,
  } = useWeeklyAvailability(user.id)
  const {
    data: exceptions,
    isLoading: isLoadingExceptions,
    error: errorExceptions,
  } = useAvailabilityExceptions(user.id)
  const {
    data: bookingsData,
    isLoading: isLoadingBookings,
    error: errorBookings,
  } = useBookings({ coachUserId: user.id, limit: 1000 })
      ...
      exceptions.forEach((ex) => {
        ...
        if (ex.isUnavailable) {
          allEvents.push({
            ...
            type: 'BLOCKAGE',
            title: ex.startTime
              ? `Blockage: ${ex.startTime} - ${ex.endTime}`
              : 'Time Off (All Day)',
            startTime: ex.startTime
              ? parseISO(`${ex.date.split('T')[0]}T${ex.startTime}`)
              : startOfDay(date),
            endTime: ex.endTime
              ? parseISO(`${ex.date.split('T')[0]}T${ex.endTime}`)
              : endOfDay(date),
          })
    ...
    if (weekly) {
      ...
      daysInRange.forEach((day) => {
        ...
        const hasException = exceptions?.some(
          (ex) => format(parseISO(ex.date), 'yyyy-MM-dd') === dateStr
        )
  ...
  if (errorWeekly || errorExceptions || errorBookings)
    return <ErrorAlert message="Failed to load calendar data." />
